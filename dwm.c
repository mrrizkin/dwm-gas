...
static int getstatustextwidth(char* text);
...
drawstatusbar(Monitor *m, int bh, char* stext, int stw) {
    ...
    w = getstatustextwidth(stext) + horizpadbar;
	text = p;
...
updatesystray(void)
{
	...
	unsigned int sw = getstatustextwidth(stext) + systrayspacing;
	unsigned int w = 1;
